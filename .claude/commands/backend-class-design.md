...
argument-hint: [対象スコープ（例: F-03 API管理、F-01 認証）省略時はdocs/design/basic/の全機能]
---

ForgeHub のバックエンド（Java/Spring Boot）クラス設計ドキュメントを作成/更新する。対象スコープ: $ARGUMENTS （省略時は `docs/design/basic/*.md` の既存詳細設計書がある機能を優先度順〔F-01→F-05〕に対象とする）。
