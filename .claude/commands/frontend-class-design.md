...
argument-hint: [対象スコープ（例: F-03 API管理、S-04 API一覧画面）省略時はdocs/design/の全機能]
---

ForgeHub のフロントエンド（TypeScript/Next.js）クラス設計ドキュメントを作成/更新する。対象スコープ: $ARGUMENTS （省略時は `docs/design/*.md` の既存詳細設計書がある機能を優先度順〔F-01→F-05〕に対象とする）。
...
対応するバックエンドクラス設計書（`docs/design/<機能スラッグ>-backend-class.md`）が既にあれば、plannerはDTO形状の整合に参照する。バックエンド側が未作成でも詳細設計書のAPI章から進めてよい（ブロックしない）。
