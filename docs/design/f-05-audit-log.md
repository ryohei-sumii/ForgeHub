...
| v0.2 | 2026-07-05 | 未決事項1・2を解消。F-02〜F-04の各設計書が確定事項D「業務操作と同一Txでのappend」へ改版（F-01のAUTH系はbest-effort独立Txのため対象外）され、`docs/requirements.md` 4.5（v0.4）にAUTH系5イベントの監査対象化が反映されたことを受けて、関連注記を更新（4章・7章・14章・16章） |
...
これは、F-02〜F-04の各本文に記載されていた「監査ログへの追記はDBコミット成功後に行う」という旧記述を、**同一トランザクション方式へ改める調整**である。各ドキュメントの該当箇所は本書の確定を受けて改版済みであり（F-02 8章・10.1・10.2、F-03 8章・10章、F-04 9.1・11章）、旧「コミット後に追記」の記述は残存しない。なお、F-01（AUTH系イベント）は業務行の更新を伴わないため本確定事項の対象外であり、次節の独立best-effort Tx方式による（F-01 14章にも同旨を明記済み）。
...
| 5 | 整合性／監査欠落 | 業務操作はコミット済みだが、コミット後の別処理として行う監査挿入が失敗し、「実行したが記録されていない」証跡欠落が生じる（F-02〜F-04の旧「コミット後に追記」記述の穴） | appendを業務操作と同一トランザクションへ確定し、原子性を保証した（確定事項D、「7. 追記経路・書込アーキテクチャ」参照）。旧「コミット後に追記」の記述はF-02〜F-04の改版により解消済みである。 |
...
1. **保持期間・パーティション・アーカイブ方針が未定**: Phase2で検討する。大規模時の深いoffsetページング性能もこの方針に依存する（「9. 検索インデックス・ページング」「15. スコープ境界」参照）。
2. **改ざん検知の暗号的手段の採否が未定**: ハッシュチェーン・署名等の導入可否はPhase2で検討する（「15. スコープ境界」参照）。
3. **`request_id`の採番・伝播方式が未確定**: MDC（Mapped Diagnostic Context）やフィルタによる採番・伝播方式は、`docs/requirements.md` 10.4のトレーシング設計と統合する必要があり、詳細は未確定である（「3. データモデル」参照）。
4. **AUTH系best-effort追記の失敗許容度が要レビュー**: AUTH系イベント（`LOGIN_SUCCESS`等）の記録失敗を許容する設計（best-effort独立Tx）が、セキュリティ要件上妥当かどうかは別途レビューが必要である（「7. 追記経路・書込アーキテクチャ」参照）。

（append同一Tx化の全社調整については、F-02 v0.3・F-03 v0.3・F-04 v0.2の改版により旧「コミット後に追記」の記述がすべて確定事項Dの同一Tx方式へ改められたため解消済み。F-01はAUTH系イベントのみで業務行更新を伴わないため同一Tx化の対象外であり、v0.3で独立best-effort Tx方式であることを明記済み。`docs/requirements.md` 4.5の文言更新についても、要件書v0.4でAUTH系5イベントが記録対象に追加されたため解消済み。いずれも本節から削除した。）
