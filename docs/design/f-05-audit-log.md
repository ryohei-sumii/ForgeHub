...
| v0.2 | 2026-07-05 | 未決事項1・2を解消。F-01〜F-04の各設計書が確定事項D「業務操作と同一Txでのappend」へ改版され、`docs/requirements.md` 4.5（v0.4）にAUTH系5イベントの監査対象化が反映されたことを受けて、関連注記を更新（4章・7章・14章・16章） |
...
これは、F-01〜F-04の各本文に記載されていた「監査ログへの追記はDBコミット成功後に行う」という旧記述を、**同一トランザクション方式へ改める調整**である。各ドキュメントの該当箇所は本書の確定を受けて改版済みであり（F-02 8章・10.1・10.2、F-03 8章・10章、F-04 9.1・11章）、旧「コミット後に追記」の記述は残存しない。
...
| 5 | 整合性／監査欠落 | 業務操作はコミット済みだが、コミット後の別処理として行う監査挿入が失敗し、「実行したが記録されていない」証跡欠落が生じる（F-01〜F-04の旧「コミット後に追記」記述の穴） | appendを業務操作と同一トランザクションへ確定し、原子性を保証した（確定事項D、「7. 追記経路・書込アーキテクチャ」参照）。旧「コミット後に追記」の記述はF-01〜F-04の改版により解消済みである。 |
...
（append同一Tx化の全社調整については、F-01 v0.3・F-02 v0.3・F-03 v0.3・F-04 v0.2の改版により旧「コミット後に追記」の記述がすべて確定事項Dの同一Tx方式へ改められたため解消済み。`docs/requirements.md` 4.5の文言更新についても、要件書v0.4でAUTH系5イベントが記録対象に追加されたため解消済み。いずれも本節から削除した。）
