...
| 対象 | 種別 | 目的 |
| ---- | ---- | ---- |
| name | UNIQUE（citext） | 名称の一意性保証、同時作成時の競合検出 |
...
| API_NAME_CONFLICT | 409 | `name`が既存API定義と重複（citext UNIQUE制約違反）。`DataIntegrityViolationException`を捕捉して変換する |
...
| 7 | 競合・整合性 | 同一`name`のAPI定義を同時に複数リクエストでPOSTし、二重作成を狙う | `name`列に`citext UNIQUE`制約を設け、`DataIntegrityViolationException`を捕捉して409 `API_NAME_CONFLICT`に変換する仕様とした（「3. API定義データモデル」「9. エラー設計」参照）。 |
