...
| v0.2 | 2026-07-06 | F-05バックエンドクラス設計（`docs/design/class/f-05-audit-log-backend-class.md`）確定に伴い、`AuditService`/`AuditAction`単一canonical enumの突合OPEN（13章(4)）を解消（10.4節・13章） |
...
`ApiDefinitionCommandService`・`ApiKeyCommandService`は`AuditService`（F-05オーナー）の抽象にのみ依存し、`AuditAction`語彙（`API_DEFINITION_CREATED`/`API_DEFINITION_UPDATED`/`API_DEFINITION_DELETED`/`API_KEY_ISSUED`/`API_KEY_REVOKED`の5値）はF-05側のcanonical enumを参照する（F-03側で重複定義しない）。`detail`の組立は`ApiAuditDetailFactory`が非機密フィールドのみで行い、F-05側`AuditService.append`実行時のdenylistストリップと二重防御を構成する。`AuditService`のシグネチャ・`AuditAction`単一canonical enumの最終オーナーはF-05であり、そのbackend-class設計（`docs/design/class/f-05-audit-log-backend-class.md` 6章・10章）が`append(AuditAction action, String targetType, UUID targetId, Map<String,Object> detail, UUID actorId)`シグネチャ・`@Transactional(REQUIRED)`伝播・単一`AuditAction` enum（`API_DEFINITION_*`/`API_KEY_*`の5値を含む）を確定済みである。本書の呼出前提（`AuditService`抽象への依存・語彙のF-05参照）と一致しており、設計レベルの突合は解消済みである（13章(4)参照）。
...
4. **F-05 `AuditService`/`AuditAction`単一canonical enumの最終確定（解消済み）**: `API_DEFINITION_*`/`API_KEY_*`の採録は`docs/design/basic/f-05-audit-log.md` 4章で確定済みであり、`AuditService.append`シグネチャ・`AuditAction`単一enumの最終オーナーであるF-05のbackend-class設計（`docs/design/class/f-05-audit-log-backend-class.md` 6章・10章）が`append(AuditAction, String, UUID, Map, UUID)`シグネチャ・`@Transactional(REQUIRED)`伝播・単一`AuditAction` enumを確定した。本書の呼出前提と一致しており、設計レベルの突合は解消済みである。残件は実装時に`AuditAction`第1引数をF-05 canonical enum（`com.forgehub.audit.domain.model.AuditAction`）でimportする点のみである（10.4節参照）。
...
（上記のうち#1〜#3はプラン段階の`OPEN`をそのまま引き継いだものであり、本書（クラス設計）のスコープでは解決しない。#4はF-05のbackend-class設計確定により設計レベルで解消済みであり、残件は実装時のimport切替のみである。#5〜#7はF-02側backend-class設計・`common.error`/`common`オーナーとの合意と歩調を合わせて解消される突合事項である。#8はF-01側の設計課題であり本書スコープ外である。）
</content>
