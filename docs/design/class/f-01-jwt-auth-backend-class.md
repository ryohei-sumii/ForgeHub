...
| v0.2 | 2026-07-06 | F-05バックエンドクラス設計（`docs/design/class/f-05-audit-log-backend-class.md`）確定に伴い、`AuditService.append`シグネチャ・`@Transactional(REQUIRED)`伝播・単一`AuditAction` enumの突合OPENを解消（2.5節・10.3節・13章）。残件を実装時のimport切替のみへ縮退 |
...
`AuditAction`はF-05（監査ログ）のaction語彙レジストリ（`docs/design/basic/f-05-audit-log.md` 4章）のAUTHグループと1対1で対応する。F-05のバックエンドクラス設計（`docs/design/class/f-05-audit-log-backend-class.md` 3章・10章）が単一20値`AuditAction` enum（`com.forgehub.audit.domain.model.AuditAction`）を唯一のcanonicalオーナーとして確定したため、本書のローカル5値定義は当該canonical enumのAUTHグループを表す暫定表現であり、実装時に`com.forgehub.audit.domain.model.AuditAction`をimportして参照へ切り替え、ローカルenumの重複定義は解消する。設計レベルの突合は解消済みであり、残件はこのimport切替のみである（「13. 未決事項」4参照）。
...
F-05側`AuditService`のインターフェース詳細・Tx実装（`@Transactional`伝播設定含む）はF-05のbackend-class設計書が最終オーナーであり、`docs/design/class/f-05-audit-log-backend-class.md`（6章・10章）が`append(AuditAction action, String targetType, UUID targetId, Map<String,Object> detail, UUID actorId)`シグネチャと`@Transactional(propagation = REQUIRED)`伝播を確定した。本書が呼出前提としていた`append(action, targetType, targetId, detail, actorId)`シグネチャと一致しており、`AuthEventRecorder`が非Tx下から呼び出すことで独立best-effort Txとなる本書の前提もF-05側（10章）で追認済みである。したがって設計レベルの突合は解消済みである（「13. 未決事項」4参照）。
...
4. **共有クラスの最終オーナーとの突合（設計レベルは解消済み・残件は実装時のコード集約）**: `User`エンティティ・`UserRepository`（の`User`部分）・`Role`はF-02、`AuditService`／`AuditAction`はF-05のbackend-class設計が最終オーナーである。いずれもクラス設計が確定済みであり（`docs/design/class/f-02-user-role-management-backend-class.md`、`docs/design/class/f-05-audit-log-backend-class.md`）、とくに`AuditService.append(AuditAction, String, UUID, Map, UUID)`シグネチャ・`@Transactional(REQUIRED)`伝播・単一`AuditAction` enumは本書の呼出前提と一致することが確認済みである。残るのは実装時のコードレベルの集約のみである（`User`/`Role`をF-02の`com.forgehub.user.domain`へ集約し本書は読取参照へ切替、本書ローカル`AuditAction`をF-05 canonical `com.forgehub.audit.domain.model.AuditAction`の参照へ切替）。詳細は本書「2.2 User」「2.5 AuditAction」「10.3節」参照。
