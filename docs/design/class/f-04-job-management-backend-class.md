...
public interface JobRepository {
    ...
    boolean existsById(UUID id); // deleted_atを問わない存在確認（削除済みも含む）
    Job saveAndFlush(Job j) throws JobNameConflictException;
    PageResult<Job> search(JobSearchCriteria c); // deleted_at IS NULL
}
```

`existsById`は`findActiveById`とは異なり`deleted_at`によるフィルタを行わない、削除済みジョブも含めた純粋な存在確認である。EP7（実行履歴一覧、6.4節）の親ジョブ確認専用であり、「ジョブが存在するか」のみを判定し「アクティブか」は問わない（詳細設計書EP5「完了後もDBに履歴として保持する」との整合、EP8の`findByIdAndJobId`が同様にactive判定を持たないこととも整合する）。
...
public class JobExecutionCommandService {
    ...
    public TriggerResult trigger(UUID jobId, UUID actorId)
            throws JobNotFoundException, JobExecutionInProgressException, JobParamsInvalidException {
        // job = jobRepository.findActiveById(jobId) 不在/削除済み → 404 JobNotFoundException
        // parametersSnapshot = job.parametersSnapshot()（凍結）
        // handlerRegistry.validateParameters(job.getType(), parametersSnapshot)
        //     → parameters_snapshot生成時点でのJSON Schema再検証（詳細設計書5章の3検証点目、EP2/EP4に続く再検証）。
        //       不一致は400 JobParamsInvalidException（既存jobsレコードの登録/更新時点では検証済みだが、
        //       型定義側のJSON Schema自体が事後に改版された場合の不整合を実行時にも検出するための防御的再検証である）
...
- SOLID: S（トリガー調整のみを責務とし、実行状態遷移は`JobExecution`エンティティへ、実際のハンドラ実行は`infrastructure.worker`へ、パラメータ検証は`HandlerRegistry`へそれぞれ分離する）。D（`HandlerRegistry`の抽象にのみ依存し、JSON Schema検証の具象実装（`HandlerRegistryImpl`）は知らない）。
- 実行そのもの（poller/lease/heartbeat/runner）はTx外・別クラス（8章）であり、本サービスは`job_executions`への`PENDING`挿入・トリガー時parameters再検証・監査記録のみを行う。
- 詳細設計書5章はパラメータ検証タイミングとして「EP2登録時」「EP4更新時」に加え「実行トリガー時（`parameters_snapshot`生成時点）」の3点を要求しており、本メソッドはその3点目を担う（`JobCommandService.create`/`update`が1・2点目を担う）。
...
public class JobExecutionQueryService {
    ...
    public PageResult<JobExecution> list(JobExecutionSearchCriteria c) throws JobNotFoundException {
        // jobRepository.existsById(c.jobId()) で親job存在確認（deleted_atを問わない。不在のみ → JobNotFoundException）
        //   ※findActiveByIdは使わない。削除済みジョブの実行履歴もEP5「履歴は保持する」方針に基づき参照可能とし、
        //     EP8（getExecution、active判定なし）とポリシーを揃える（4.1節参照）
...
classDiagram
    ...
    JobExecutionCommandService --> HandlerRegistry
...
public class JobJpaRepositoryAdapter implements JobRepository {
    ...
    public boolean existsById(UUID id) {
        return springDataJobRepository.existsById(id); // deleted_atによるフィルタなし（削除済みも存在扱い）
    }
...
public class JobExecutionRunner {
    ...
    private final ThreadPoolTaskExecutor handlerTaskExecutor;
    private final Clock clock;
    ...
    public JobExecutionRunner(HandlerRegistry handlerRegistry,
                               ...
                               @Qualifier("jobRunnerTaskExecutor") ThreadPoolTaskExecutor runnerTaskExecutor,
                               @Qualifier("jobHandlerTaskExecutor") ThreadPoolTaskExecutor handlerTaskExecutor,
                               Clock clock) {
        // 全依存はコンストラクタ注入。runnerTaskExecutor/handlerTaskExecutorは@Beanとして別々に定義されたbounded
        // ThreadPoolTaskExecutorであり、同一プールインスタンスを共有しない（入れ子自己枯渇デッドロックの防止。後述）
    }

    public void submit(JobExecution leased) {
        // runnerTaskExecutor（外側run(...)投入専用のbounded ThreadPoolTaskExecutor）へ run(...) を投入する
    }
    ...
    public void run(JobExecutionContext ctx) {
        ...
        // Future<ExecutionOutcome> future = handlerTaskExecutor（runnerTaskExecutorとは別プール）経由でhandler.execute(ctx)を実行
...
- **入れ子自己枯渇デッドロックの防止**: `submit(leased)`が投入する外側`run(...)`タスクと、`run`内部が`Future`経由で投入する内側`handler.execute(ctx)`タスクは、同一の`ThreadPoolTaskExecutor`を共有しない別プール（`runnerTaskExecutor`/`handlerTaskExecutor`、それぞれ別`@Bean`定義）に分離する。同一プールを両方に使い回すと、`run`側は`future.get(10, TimeUnit.MINUTES)`で自プールのスレッドを保持したままブロックするため、並行実行数がプールサイズに達すると全スレッドが`run`側でブロックし、内側`handler.execute`タスクがキューから一切拾われず最大10分間進行不能になる（自己枯渇デッドロック）。2プールに分離することで、外側`run`のブロックが内側ハンドラ実行の消費するプールを枯渇させることはなくなる。
...
| ------ | --------------- | ------------------- | -------- |
| `JOB_NOT_FOUND` | 404 | `JobNotFoundException` | 指定されたジョブIDが存在しない、または`deleted_at`が設定済み（ソフト削除済み）。ただしEP7（実行履歴一覧）は`existsById`による存在確認のみを行うため、ジョブID自体が存在しない場合にのみ本コードを送出し、削除済みジョブに対しては404にせず履歴を返却する（6.4節・4.1節参照） |
...
| `JOB_PARAMS_INVALID` | 400 | `JobParamsInvalidException` | `parameters`がJSON Schemaに不適合、または8KBを超過（EP2登録時・EP4更新時に加え、EP6トリガー時の`parameters_snapshot`生成時点の再検証でも送出され得る。詳細設計書5章の3検証点と対応） |
...
sequenceDiagram
    ...
    alt 存在しない、またはソフト削除済み
        ...
        CS->>HR: validateParameters(job.getType(), parametersSnapshot)
        alt JSON Schema不適合または8KB超過
            HR-->>CS: JobParamsInvalidException
            CS-->>C: 400 JOB_PARAMS_INVALID（rollback）
        else 検証OK
...
| `application`（`JobQueryService`/`JobExecutionQueryService`） | `get`/`getExecution`の`JobNotFoundException`/`JobExecutionNotFoundException`送出、`findByIdAndJobId`が`job_id`不一致時にemptyを返すモック挙動でのIDOR回帰防止を検証する。`JobExecutionQueryService.list`は`existsById`が`false`の場合のみ`JobNotFoundException`を送出し、削除済みジョブ（`existsById`は`true`）では例外を送出せず履歴を返却することを検証する（EP7/EP8ポリシー整合の回帰防止）。 |
...
| 18 | 詳細設計書整合／トリガー時パラメータ検証欠落 | 詳細設計書5章が要求する「実行トリガー時（`parameters_snapshot`生成時点）」のJSON Schema再検証を`JobExecutionCommandService.trigger`が欠落させ、EP6経由では常に無検証で`parameters_snapshot`が生成されてしまう。 | `trigger`内で`handlerRegistry.validateParameters(job.getType(), parametersSnapshot)`を呼び出し、EP2登録時・EP4更新時に続く3点目の検証点として明示した。不適合時は400 `JobParamsInvalidException`とする（6.3節・12.3節・13.4節参照）。 |
| 19 | トランザクション／実行基盤（Executorの入れ子自己枯渇） | `JobExecutionRunner`が単一の`ThreadPoolTaskExecutor`を「外側`run`タスクの投入」と「内側`handler.execute`の投入」の両方に用いると、`run`側の`future.get(10分)`ブロックによりプールが枯渇し、内側ハンドラ実行が最大10分間ピックアップされなくなる（15.2#4の行ロック長時間占有対策では回避できない、プール入れ子固有の懸念）。 | `runnerTaskExecutor`（外側`run`投入専用）と`handlerTaskExecutor`（内側`handler.execute`投入専用）を別`@Bean`のbounded `ThreadPoolTaskExecutor`として分離し、同一プールを両用途で共有しない構成とした（8.3節参照）。 |
| 20 | 詳細設計書整合／削除済みジョブの履歴参照非対称 | EP7（`JobExecutionQueryService.list`）が`jobRepository.findActiveById`で親ジョブのactive判定を行い削除済みジョブの履歴一覧を404にする一方、EP8（`getExecution`）は`findByIdAndJobId`のみでactive判定を持たず削除済みジョブの実行詳細でも200を返しており、詳細設計書EP5「完了後もDBに履歴として保持する」との整合が取れていなかった。 | `JobRepository`に`existsById`（`deleted_at`を問わない存在確認）を追加し、EP7の親ジョブ確認を`existsById`ベースへ変更した。これにより「存在しないjobId」のみ404 `JOB_NOT_FOUND`とし、削除済みジョブの履歴はEP7・EP8のいずれからも参照可能となり、EP5の履歴保持方針と整合する（4.1節・6.4節・7.1節参照）。 |
...
5. **`JobExecutionRunner`のbounded `ThreadPoolTaskExecutor`サイズ・pollerの空き容量判定閾値**: `runnerTaskExecutor`/`handlerTaskExecutor`を別プールに分離する構成自体は本書で確定済み（入れ子自己枯渇デッドロックの防止、8.3節・15.2節#19参照）だが、各プールの具体的なコアサイズ・最大サイズ・キュー長は実装時のチューニング事項であり、本書では具体数値を確定しない（8.1節・8.3節参照）。
