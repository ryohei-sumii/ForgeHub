...
4. **F-05 `AuditService`の正式シグネチャとTx伝播の突合**: F-05の正式パッケージ・`append(action, targetType, targetId, detail, actorId)`シグネチャ・`@Transactional(REQUIRED)`伝播設定は、F-05側backend-class設計が最終オーナーである。本書は同一Tx参加を前提として`UserCommandService`を設計しているが、F-05側確定時の突合を要する（「6.3」参照）。
