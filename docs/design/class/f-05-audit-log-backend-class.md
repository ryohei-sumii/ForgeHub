...
public class AuditQueryService {
    ...
    public PageResult<AuditLogView> search(AuditSearchCommand cmd) throws AuditValidationException {
        ...
        // size==null（省略）→ 20をデフォルト適用。size!=nullかつ(size<0 または size>100) → AuditValidationException
...
- 検証: `actorId`/`targetId`のuuid parse不正、`action`/`targetType`のenum外、`from>to`、`size`範囲外はすべて`AuditValidationException`（400）へ集約する（呼出元Controllerでの`MethodArgumentTypeMismatchException`由来500を回避する。11章参照）。`from`省略時は`now-30日`、`to`省略時は`now`をデフォルト適用する。`size`省略時（`null`）は20をデフォルト適用し、明示指定された値のみ`size<0`または`size>100`の範囲チェック対象とする（`size`をプリミティブ`int`のままにすると省略時の初期値0と明示的な`size=0`指定が区別できず、既定20が機能しなくなるための措置。8.2節参照）。
...
public record AuditSearchCommand(
        ...
        String from, String to, int page, Integer size) {
}
```

依存: なし。`AuditLogSearchRequest`（presentation.dto、8章）から生String（型変換前）のまま受け渡す。`size`は`AuditLogSearchRequest`と同様に`Integer`（nullable）とし、省略時の既定20適用は`AuditQueryService.search`に集約する（`page`はプリミティブ`int`のままでよい。省略時の初期値0が既定値0と一致するため問題を生じない）。
...
public record AuditLogSearchRequest(
    ...
    // page既定0（プリミティブintの初期値0と一致するため省略時バインドでも既定0が成立する）
    // sizeは既定20だが、プリミティブintでは省略時に0へバインドされ既定20を表現できないためInteger化する。
    // 省略時（size==null）は`AuditQueryService`側でデフォルト20を適用する（6.3節参照）。
}
```

備考: uuid/enum/from-toの意味検証はここでは行わず、`AuditQueryService`で実施し`AuditValidationException`へ集約する（型mismatch回避のため生Stringのまま受理する。6.3節参照）。`size`が`null`（省略）の場合の既定値適用も同様に`AuditQueryService`側に集約し、本DTOでは型（`Integer`）のみを担保する。
...
| 8 | 詳細設計書整合／`size`デフォルト20の不整合 | `AuditLogSearchRequest.size`をプリミティブ`int`のまま受けると、クエリ省略時にint既定値0へバインドされ、詳細設計書5章が定める「`size`デフォルト20」が機能せず省略時に空ページが返る（`AuditQueryService`の`size<0または size>100`検証は`size=0`を素通りさせるため検出できない）。 | `AuditLogSearchRequest`/`AuditSearchCommand`の`size`を`Integer`（nullable）化し、`size==null`（省略）時に`AuditQueryService.search`側でデフォルト20を適用する構成とした。明示的な`size=0`指定とは区別され、既定20が実際に機能する（6.3節・6.4節・8.2節参照）。 |
...
| `application`（`AuditQueryService`） | `AuditLogReadRepository`をモック化し、`from`/`to`省略時のデフォルト適用（30日/now）、`size`省略時（`null`）にデフォルト20が適用されること（`size=0`明示指定時は0のまま範囲内として扱われ20へ補正されないこと）、`from>to`・`size`範囲外・uuid/enum不正時の`AuditValidationException`、`get`不在時の`AuditNotFoundException`を検証する。 |
