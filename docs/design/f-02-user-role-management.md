...
監査ログ`USER_DISABLED`はDBコミット直後、Redis操作の成否に関わらず記録する（Redis不通で503を返す場合でも、無効化の事実自体はDBコミット済みであり監査対象であるため。`docs/requirements.md` 4.2「無効化は監査ログに記録される」準拠）。
...
処理の骨子は無効化フローと同様であり、(1)新パスワードのbcrypt再格納をコミット、(2)監査ログ`USER_PASSWORD_RESET`をコミット直後に記録、(3)`rtuser:{userId}`索引を用いて当該ユーザーの全RTを削除し旧セッションを遮断する。Redis不通時は同様に503（`USER_SESSION_PURGE_FAILED`）を返却し、Adminに再実行を促すが、パスワード再格納と監査ログは既に確定済みである。
...
3. **F-05監査ログスキーマとの最終整合確認**: 本書で定義した`USER_*`アクション語彙（`USER_CREATED`等）および`detail`のbefore/after形式が、F-05（監査ログ）の設計文書側と最終的に整合しているかどうかは未確認である（「8. 監査ログ（F-05連携）」参照）。

（無効化時のRT一括失効については、F-01に`rtuser:{userId}`索引を追加したことで解消済みのため本節から削除した。「10.1 無効化フロー」「12. 設計上の検討事項（敵対評価）」#6参照。）
