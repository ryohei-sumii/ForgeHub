...
| v0.3 | 2026-07-05 | F-05設計書（`docs/design/f-05-audit-log.md`）の確定を反映。監査ログ追記を「DBコミット成功後」から業務操作と同一Txへ変更（確定事項D準拠、8章・10.1・10.2）。`USER_*`アクション語彙・`detail`形式のF-05整合を確定しOPENを解消（13章） |
...
制約として、ロール変更・アカウント無効化を行っても、発行済みのATは最大15分間（F-01のAT TTL）有効なまま残る。これはATがステートレスであり個別失効ができないためであり、`docs/design/f-01-jwt-auth.md` 11章L230および同書「未決事項1」と同一の制約である（※本項目は未決。詳細は末尾「13. 未決事項」参照）。この残存リスクに対しては、無効化時に`rtuser:{userId}`索引（`docs/design/f-01-jwt-auth.md` 5章・11章）経由でRTを全削除しリフレッシュ経路を遮断することで、Redis到達可能時はrefreshによるAT再取得を完全に防止する（詳細は「10. F-01/F-05整合フロー」参照）。
...
本章で定義した`USER_*`アクション語彙および`detail`のbefore/after形式は、F-05側のaction語彙レジストリ（`docs/design/f-05-audit-log.md` 4章）にcanonical（正典）として採録され、整合確認済みである。
...
監査ログ`USER_DISABLED`は`enabled=false`の更新と**同一DBトランザクション**で挿入・コミットし、Redis操作の成否に関わらず確定させる（Redis不通で503を返す場合でも、無効化の事実と監査記録はDB上で原子的に確定済みである。`docs/requirements.md` 4.2「無効化は監査ログに記録される」および`docs/design/f-05-audit-log.md` 7章 確定事項D準拠）。

DB更新は既にコミットされているため、Redis不通時もアカウント無効化自体は成立している。Redisが到達可能であれば`rtuser:{userId}`索引（`docs/design/f-01-jwt-auth.md` 5章・11章参照）により当該ユーザーの全familyを漏れなく削除でき、以降の`/api/v1/auth/refresh`は即座に`401 AUTH_REFRESH_REVOKED`となるため、既存RTを用いたAT再取得は成立しない。Redis不通時のみ、削除完了までの間は既存RTによるrefreshが理論上成功しうる残存ギャップが生じるため、503を返しAdminに再実行を促す。この場合も新規ログイン（`/api/v1/auth/login`）は`enabled`をDB照合済みのため拒否される。

残る既知の制約は、Redis到達可能時であっても発行済みATは最大15分間（F-01のAT TTL）有効なまま残ることのみであり、これは10.3のロール変更と同一の、ATのステートレス性に起因する制約である（`docs/design/f-01-jwt-auth.md`「AT失効の限界」参照）。

### 10.2 パスワードリセットフロー（EP7: reset-password）

処理の骨子は無効化フローと同様であり、(1)新パスワードのbcrypt再格納と監査ログ`USER_PASSWORD_RESET`の挿入を同一Txで同時にコミットし（`docs/design/f-05-audit-log.md` 7章 確定事項D準拠）、(2)`rtuser:{userId}`索引を用いて当該ユーザーの全RTを削除し旧セッションを遮断する。Redis不通時は同様に503（`USER_SESSION_PURGE_FAILED`）を返却し、Adminに再実行を促すが、パスワード再格納と監査ログは既に確定済みである。
...
（無効化時のRT一括失効については、F-01に`rtuser:{userId}`索引を追加したことで解消済みのため本節から削除した。「10.1 無効化フロー」「12. 設計上の検討事項（敵対評価）」#6参照。F-05監査ログスキーマとの最終整合確認については、`USER_*`アクション語彙・`detail`のbefore/after形式が`docs/design/f-05-audit-log.md` 4章のaction語彙レジストリにcanonicalとして採録され、追記方式も同7章 確定事項D「業務操作と同一Tx」に改めたことで解消済みのため本節から削除した。「8. 監査ログ（F-05連携）」参照。）
