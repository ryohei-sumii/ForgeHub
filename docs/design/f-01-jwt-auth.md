...
| v0.3 | 2026-07-05 | F-05設計書（`docs/design/f-05-audit-log.md`）の確定を反映。認証イベント5種の監査記録が確定事項Aとして正式に監査対象へ承認され`docs/requirements.md` 4.5にも反映されたため、F-05整合のOPENを解消。追記は独立したbest-effort Txで行うことを明記（14章・16章） |
...
- **F-05（監査ログ）**: 以下のアクションを`AUDIT_LOG`テーブル（`actor_id`, `action`, `target_type=USER`, `detail`(jsonb)）へ追記する。これにより`docs/requirements.md` 4.1の「ログイン失敗のログ記録」要件を満たす。
  ...
  これらの認証イベント5種の監査ログ記録は、`docs/design/f-05-audit-log.md` 4章の確定事項Aとして正式に監査対象へ含めることが承認され、`docs/requirements.md` 4.5（v0.4）にも記録対象として反映済みである。また、これらは業務行の更新を伴わない（`users`テーブルへの書込がない）イベントであるため、F-05の定め（同7章「例外: AUTH系イベントの独立best-effort Tx」）に従い、独立した短命のINSERTトランザクションでbest-effort記録する。記録に失敗しても認証処理本体（ログイン成否等）には影響させない。
...
（認証イベントの監査ログ記録とF-05設計との整合については、`docs/design/f-05-audit-log.md` 4章の確定事項Aとして承認され、`docs/requirements.md` 4.5にも反映されたことで解消済みのため本節から削除した。「14. 他機能依存」参照。）
