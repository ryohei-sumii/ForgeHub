...
| family（familyId） | 同一ログイン起源のRT系列を束ねる識別子。ローテーションを繰り返しても同一familyIdを引き継ぐ。 |
...
| 項目 | 内容 |
| ---- | ---- |
| AT保存先 | 保存しない（完全ステートレス）。クライアント側でJSメモリ変数として保持する。 |
...
design-doc-plannerによる敵対的レビュー（設計への攻撃的評価）の結果と、それぞれへの対処を以下に示す。実装者はこの章のみを読んでも「何を懸念し、どう対処したか」を把握できるようにしてある。

| # | 観点 | 想定シナリオ（攻撃・懸念） | 対処（RESOLVED） |
| - | ---- | -------------------------- | ----------------- |
| 1 | セキュリティ（認可漏れ） | `@PreAuthorize`の付け忘れがあったエンドポイントが、認証済みであれば全ロールで通過してしまう。 | `SecurityConfig`側でデフォルトを`authenticated`必須・deny-by-defaultとし、注釈忘れによる過剰許可を防止する（「8. 認可(RBAC)実装」参照）。 |
| 2 | セキュリティ（algの混入） | 攻撃者が`alg=none`や、RS256用の公開鍵をHS256の共有鍵として悪用し、署名を偽造する。 | JWTデコーダの許可アルゴリズムをHS256限定とし、それ以外のalgを含むトークンを検証前に拒否する（「3. 認証方式決定」「11. セキュリティ制御」参照）。 |
| 3 | セキュリティ（ユーザー列挙） | 存在しないメールアドレスは即時401、存在するメールアドレスはbcrypt照合分の遅延を伴う401となり、応答時間差からアカウントの存否が推測できてしまう。 | 存在しないメールアドレスに対してもダミーのbcrypt演算を実行して応答時間を平準化し、エラーコードも`AUTH_INVALID_CREDENTIALS`に統一する（「9.1」「11. セキュリティ制御」参照）。 |
| 4 | セキュリティ（トークン窃取） | XSSにより`localStorage`に保存されたRTが窃取され、永続的な侵害につながる。 | RTは`httpOnly`Cookieに保存し、ATのみJSメモリに保持することでXSSによる露出を最小化する。加えて、RTローテーションと再利用検知により、窃取後にRTが永続的に悪用されることを防止する（「5. トークン保存先」「11. セキュリティ制御」参照）。 |
| 5 | セキュリティ（権限降格の未反映） | Adminがユーザーを無効化・ロール変更しても、発行済みのATは有効期限内（最大15分）APIアクセスを継続できてしまう。 | ユーザー無効化時に該当ユーザーの全RTを即時削除し、リフレッシュ経路を遮断する。ただしAT自体はステートレスなため即時失効はできず、最大15分の残存アクセス窓は許容する（**未決事項として引き続き残存**。詳細は「11. セキュリティ制御」および末尾「未決事項」参照）。 |
| 6 | 競合整合性（RTローテーション競合） | 複数タブから同時にrefreshが行われ、一方がローテーション済みの旧RTを提示した場合、再利用検知によって家族全体が失効させられ、正規ユーザーが強制ログアウトされてしまう。 | grace期間60秒を設け、ローテーション直後の旧RTを一度だけ正当なものとして許容する（`rotatedFrom`記録により追跡）。これにより誤検知による強制ログアウトを回避する（「9.2」参照）。 |
| 7 | 競合整合性（二重ログイン） | 同一ユーザーが複数端末からログインした場合、RT管理が衝突する可能性がある。 | jti単位でのRT管理に加え、familyIdを端末（ログインセッション）単位で分離することで、端末ごとに独立した失効が可能な構成とする（「5. トークン保存先」参照）。 |
| 8 | スケール性能（ログイン遅延） | bcryptのコストを高く設定すると、ログインのp95応答時間目標を超過する恐れがある。 | cost=10で概ね50〜100ms程度に収まり、かつログイン自体の呼び出し頻度は低いため、p95 500ms以内の目標に影響しない（「12. 非機能」参照）。 |
| 9 | スケール性能（検証時のDB負荷） | 通常API呼び出しのたびにロール検証でDBを参照すると、N+1的な負荷増大を招く。 | ATはステートレスであり、通常のAPI検証は署名検証のみで完結しDBを参照しない（「4. トークン設計」「9.3」「12. 非機能」参照）。 |
| 10 | 境界異常系（Redis障害） | Redisが停止すると、refreshが実行不能になり状態不整合を招く恐れがある。 | Redis不通時はlogin/refreshを503でフェイルクローズする。一方、AT単体の検証処理はRedisに依存しないため、通常API呼び出しは継続可能とする（「10. エラー設計」参照）。 |
| 11 | 境界異常系（クロックスキュー） | サーバ間の時刻ずれにより、`exp`境界付近の正当なトークンが誤って拒否されてしまう。 | 30秒のleewayを許容する（「4. トークン設計」参照）。 |
| 12 | スコープ逸脱（アカウントロック） | ログイン失敗回数に基づく制御を作り込むと、MVPスコープを逸脱する。 | `docs/requirements.md` 4.1に準拠し、Phase1では失敗のログ記録のみに限定し、ロック機構自体は実装しない（「15. スコープ境界」参照）。 |

## 17. 未決事項

以下は design-doc-planner の敵対評価においても解決に至らず、`OPEN`として残された事項である。実装・レビュー時には特に注意すること（各該当章の本文中にも同様の注記を配置済み）。

1. **AT無効化不可による権限降格・無効化反映の遅延（最大15分）**: ロール変更やアカウント無効化を行っても、既に発行済みのATは最大15分間有効なまま残る。短命化（15分TTL）によりリスクは緩和しているが、完全な即時失効を実現するには、Phase2でATのブラックリストをRedis等に導入することを検討する（「11. セキュリティ制御」「16. 設計上の検討事項（敵対評価）」#5参照）。
