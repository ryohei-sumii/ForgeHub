...
| (status, started_at) | 複合インデックス | reconcilerによる最大実行時間超過（生存ワーカーの暴走）検出の高速化 |
...
- **reconciler**: `@Scheduled`による30秒間隔、および起動時（アプリ起動直後）の実行により、以下2条件のいずれかに該当するレコードを`TIMED_OUT`へ回収する（`finished_at=now()`）。
    - 条件1（lease失効）: `status=RUNNING`かつ`heartbeat_at < now() - 60秒`（`error_reason=LEASE_EXPIRED`）。Cloud Runインスタンスのスケールイン等によりワーカーが消滅した場合に、実行が永久に`RUNNING`のまま滞留してしまう事態（孤児実行）を防ぐための仕組みである。
    - 条件2（最大実行時間超過）: `status=RUNNING`かつ`started_at < now() - 10分`（`error_reason=EXECUTION_TIMEOUT`）。ワーカーが生存し続けheartbeatが15秒ごとに更新され続けている（＝条件1では検出できない）暴走ハンドラであっても、実行開始から10分を超えた場合はこの条件により強制的に回収する。
- **最大実行時間**: 10分を超える実行は`TIMED_OUT`とする。ハンドラ自体が自発的に処理を打ち切る実装をしている場合はハンドラ側のタイムアウト処理が先に効くが、ハンドラが自発的に打ち切らず生存し続ける場合でも、reconcilerの条件2（`started_at`基準）により必ず10分超過時点で強制的に`TIMED_OUT`へ回収されることを保証する。
...
実行履歴一覧。クエリパラメータ: `page`（デフォルト0）、`size`（デフォルト20、最大100）、`status`（フィルタ）。`created_at`（enqueue＝`PENDING`挿入時点、「4.1」参照）降順でソートする。`started_at`は`RUNNING`遷移時まで設定されないため`PENDING`実行では常にNULLとなり、ソートキーに用いるとNULLの並び順（DBのデフォルト挙動）に依存した未定義に近い挙動になる。`created_at`はNOT NULLであるため、この問題が発生せず、直近にトリガーした実行（`PENDING`を含む）を常に先頭に表示できる。
...
sequenceDiagram
    ...
    S->>D: status=RUNNING かつ heartbeat_at < now()-60s を検索（条件1: lease失効）
    alt 該当あり
        S->>D: status=TIMED_OUT, finished_at=now(), error_reason=LEASE_EXPIRED を UPDATE
    end
    S->>D: status=RUNNING かつ started_at < now()-10分 を検索（条件2: 最大実行時間超過）
    alt 該当あり
        S->>D: status=TIMED_OUT, finished_at=now(), error_reason=EXECUTION_TIMEOUT を UPDATE
    else いずれの条件にも該当なし
        S->>S: 何もしない
    end
```
