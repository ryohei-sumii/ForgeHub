...
| v0.2 | 2026-07-04 | design-doc-reviewerの指摘を反映。F-01に追加された`rtuser:{userId}`索引を用いてユーザー単位のRT一括失効を正しく実現するよう無効化・パスワードリセットフローを修正、監査ログ記録の欠落を解消、エラーコードの命名を整理（9章・10章・12章・13章） |
| v0.3 | 2026-07-05 | F-05設計書（`docs/design/basic/f-05-audit-log.md`）の確定を反映。監査ログ追記を「DBコミット成功後」から業務操作と同一Txへ変更（確定事項D準拠、8章・10.1・10.2）。`USER_*`アクション語彙・`detail`形式のF-05整合を確定しOPENを解消（13章） |
...
`/users/me` は本来F-01（認証）に近い機能に見えるが、`docs/design/basic/f-01-jwt-auth.md`（7章、L104）にて「F-02の管轄とし、本書のスコープ外とする」と明記されているため、本書F-02側で設計する。
...
参照要件: `docs/requirements.md` 4.2（ユーザー・ロール管理）、7章（API設計方針）、`docs/design/basic/f-01-jwt-auth.md` 6章・7章・11章。
...
ロール値の表現方針: DBには`'Admin'` / `'Developer'` / `'Operator'`の文字列をそのまま格納する。Spring Security側の権限表現（`ROLE_ADMIN`等）へは、`docs/design/basic/f-01-jwt-auth.md` 8章（L109）と整合する形でアプリケーション層がマッピングを行う。DB格納値そのものを`ROLE_`プレフィックス付きに変更することはしない。
...
本エンドポイントはAdmin主導のリセットであり、F-01がスコープ外としているself-serviceのパスワードリセット（`docs/design/basic/f-01-jwt-auth.md` 4.1参照）とは別物である。ユーザー自身が忘れた場合のセルフリセット手段はMVPには存在しない。
...
- 許可される値は`Admin` / `Developer` / `Operator`の3種に固定する（`docs/design/basic/f-01-jwt-auth.md` 6章L90と整合）。
...
認可方式は`docs/design/basic/f-01-jwt-auth.md` 8章のRBAC実装をそのまま踏襲する。`@PreAuthorize("hasRole('ADMIN')")`アノテーションにより、`/api/v1/users`系のすべてのエンドポイント（`/users/me`を除く）をADMINロール限定とする。`/users/me`のみ「認証済みであれば全ロール可」とする。

デフォルトdeny原則もF-01と同一とする。注釈のないエンドポイントは`SecurityConfig`側で「認証済みであること」をデフォルト要求し、注釈忘れによる意図しない全許可を防ぐ（`docs/design/basic/f-01-jwt-auth.md` 8章L121参照）。

操作実行者（actor）の特定は、検証済みATの`SecurityContext`上の`sub`クレームのみを用い、DBへの再照会は行わない（`docs/design/basic/f-01-jwt-auth.md` 9.3節SEQ_verify、L197と整合）。

制約として、ロール変更・アカウント無効化を行っても、発行済みのATは最大15分間（F-01のAT TTL）有効なまま残る。これはATがステートレスであり個別失効ができないためであり、`docs/design/basic/f-01-jwt-auth.md` 11章L230および同書「未決事項1」と同一の制約である（※本項目は未決。詳細は末尾「13. 未決事項」参照）。この残存リスクに対しては、無効化時に`rtuser:{userId}`索引（`docs/design/basic/f-01-jwt-auth.md` 5章・11章）経由でRTを全削除しリフレッシュ経路を遮断することで、Redis到達可能時はrefreshによるAT再取得を完全に防止する（詳細は「10. F-01/F-05整合フロー」参照）。

## 6. パスワード取扱

- 初期パスワードはシステムがCSPRNGにより自動生成する24文字の文字列とし、Adminが任意の文字列を入力する経路は提供しない。これは弱いパスワードの混入や、リクエストボディ経由での平文パスワードのアクセスログ混入を避けるためである。
- 格納時は`docs/design/basic/f-01-jwt-auth.md`と同じくbcrypt（cost=10）でハッシュ化する。平文パスワードはDB・アプリケーションログ・例外メッセージ・監査ログの`detail`のいずれにも一切出力しない（`docs/requirements.md` 10.1準拠）。
...
本章で定義した`USER_*`アクション語彙および`detail`のbefore/after形式は、F-05側のaction語彙レジストリ（`docs/design/basic/f-05-audit-log.md` 4章）にcanonical（正典）として採録され、整合確認済みである。
...
| USER_SESSION_PURGE_FAILED | 503 | アカウント無効化またはパスワードリセット時に、Redis不通で当該ユーザーのRT一括失効（`rtuser:{userId}`索引の走査）に失敗 |
| AUTH_FORBIDDEN | 403 | ロール不足による認可失敗（F-01のコードを再利用） |
| AUTH_UNAUTHENTICATED | 401 | 未認証（F-01のコードを再利用） |

レスポンスボディは`docs/requirements.md` 7章および`docs/design/basic/f-01-jwt-auth.md` 10章と同様に`{code, message, details}`形式で統一する。
...
sequenceDiagram
    ...
    alt 最終有効Adminに該当
        ...
        U->>D: enabled=false, disabled_at=now() 更新（未コミット）
        U->>L: USER_DISABLED 追記（同一Tx内）
        U->>D: 業務更新 + 監査INSERT を同時にコミット
        D-->>U: コミット成功
        U->>R: rtuser:{userId} をSMEMBERSで列挙
        loop 各familyId
            U->>R: rtfam:{familyId}配下の全RT削除 + rtfam:{familyId}削除
        end
        U->>R: rtuser:{userId} 削除
        alt Redis到達可能
            R-->>U: 削除成功
            U-->>C: 200 OK
        else Redis不通
            R-->>U: 削除失敗
            U-->>C: 503 USER_SESSION_PURGE_FAILED
            Note over U: DB上のenabled=falseと監査ログUSER_DISABLEDは既に確定済み（再実行を促す）
        end
    end
```

監査ログ`USER_DISABLED`は`enabled=false`の更新と**同一DBトランザクション**で挿入・コミットし、Redis操作の成否に関わらず確定させる（Redis不通で503を返す場合でも、無効化の事実と監査記録はDB上で原子的に確定済みである。`docs/requirements.md` 4.2「無効化は監査ログに記録される」および`docs/design/basic/f-05-audit-log.md` 7章 確定事項D準拠）。

DB更新は既にコミットされているため、Redis不通時もアカウント無効化自体は成立している。Redisが到達可能であれば`rtuser:{userId}`索引（`docs/design/basic/f-01-jwt-auth.md` 5章・11章参照）により当該ユーザーの全familyを漏れなく削除でき、以降の`/api/v1/auth/refresh`は即座に`401 AUTH_REFRESH_REVOKED`となるため、既存RTを用いたAT再取得は成立しない。Redis不通時のみ、削除完了までの間は既存RTによるrefreshが理論上成功しうる残存ギャップが生じるため、503を返しAdminに再実行を促す。この場合も新規ログイン（`/api/v1/auth/login`）は`enabled`をDB照合済みのため拒否される。

残る既知の制約は、Redis到達可能時であっても発行済みATは最大15分間（F-01のAT TTL）有効なまま残ることのみであり、これは10.3のロール変更と同一の、ATのステートレス性に起因する制約である（`docs/design/basic/f-01-jwt-auth.md`「AT失効の限界」参照）。

### 10.2 パスワードリセットフロー（EP7: reset-password）

処理の骨子は無効化フローと同様であり、(1)新パスワードのbcrypt再格納と監査ログ`USER_PASSWORD_RESET`の挿入を同一Txで同時にコミットし（`docs/design/basic/f-05-audit-log.md` 7章 確定事項D準拠）、(2)`rtuser:{userId}`索引を用いて当該ユーザーの全RTを削除し旧セッションを遮断する。Redis不通時は同様に503（`USER_SESSION_PURGE_FAILED`）を返却し、Adminに再実行を促すが、パスワード再格納と監査ログは既に確定済みである。

### 10.3 ロール変更時のAT反映遅延

ロール変更（EP4）では、無効化と異なりRTの削除は行わない（再ログインを要求しない）。ただし、既に発行済みのATには変更前の`role`クレームが最大15分間残存する。これは`docs/design/basic/f-01-jwt-auth.md` 11章の「AT失効の限界」および同書「未決事項1」と同一の既知の制約である（※本項目は未決。詳細は末尾「13. 未決事項」参照）。即時反映が必要な場合はPhase2でのATブラックリスト導入等が候補となるが、本書のスコープでは対応しない。
...
| 6 | 競合・整合性 | disableでDB上`enabled=false`とした後、Redis上のRT削除が漏れ、無効化済みユーザーがrefreshでAT再取得を継続する | F-01に`rtuser:{userId}`索引を追加し、無効化フローがDBコミット後に当該ユーザーの全family（全RT）を漏れなく削除できる構成とした。Redis到達可能時はrefreshが即座に`401 AUTH_REFRESH_REVOKED`となり解消済み。Redis不通時のみ503 `USER_SESSION_PURGE_FAILED`を返却してAdminへ再実行を促す（この場合の残存ギャップはRedis復旧までの間に限定される。「10.1 無効化フロー」参照）。 |
...
上記のうち#6はF-01への`rtuser:{userId}`索引追加により解消済みである。Redis不通時のみ復旧までの残存ギャップが生じるが、これは他の外部依存障害時と同様の一般的な可用性上の制約であり、本書独自の未決事項としては扱わない。
...
2. **ロール変更後のAT反映遅延（最大15分）**: ロール変更を行っても、発行済みATは最大15分間旧`role`のまま有効である。これは`docs/design/basic/f-01-jwt-auth.md`の未決事項1と同一の制約であり、AT即時失効を実現するにはPhase2でのブラックリスト導入等の検討が必要である（「5. Admin認可とF-01整合」「10.3 ロール変更時のAT反映遅延」参照）。
（無効化時のRT一括失効については、F-01に`rtuser:{userId}`索引を追加したことで解消済みのため本節から削除した。「10.1 無効化フロー」「12. 設計上の検討事項（敵対評価）」#6参照。F-05監査ログスキーマとの最終整合確認については、`USER_*`アクション語彙・`detail`のbefore/after形式が`docs/design/basic/f-05-audit-log.md` 4章のaction語彙レジストリにcanonicalとして採録され、追記方式も同7章 確定事項D「業務操作と同一Tx」に改めたことで解消済みのため本節から削除した。「8. 監査ログ（F-05連携）」参照。）
